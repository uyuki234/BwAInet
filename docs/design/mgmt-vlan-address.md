# 管理 VLAN (VLAN 11) アドレスアサイン表

## 基本情報

| 項目 | 値 |
|------|-----|
| VLAN ID | 11 |
| サブネット | 192.168.11.0/24 |
| サブネットマスク | 255.255.255.0 |
| デフォルトゲートウェイ | 192.168.11.1 |
| IPv6 | なし (v4 only) |
| 用途 | NW 機器管理、DNS/DHCP サーバー |

## 静的割り当て (.1 – .49)

| アドレス | ホスト名 | 機器 / 役割 | 備考 |
|---------|---------|------------|------|
| 192.168.11.1 | r3-vyos | VyOS ルーター (VLAN 11 GW) | DNS/DHCP/BGP/NetFlow |
| 192.168.11.2 | local-srv | ローカルサーバー CT | Grafana, rsyslog, nfcapd, SNMP Exporter |
| 192.168.11.3 | proxmox | Proxmox VE ホスト (Dell OptiPlex 3070 Micro) | 管理 WebUI (8006/tcp) |
| 192.168.11.4 | sw01 | PoE スイッチ (FS) | VLAN 11/30/40 トランク |
| 192.168.11.5 | wlc01 | Cisco 3504 WLC | Aironet 3800 AP 管理、QoS 制御 |
| 192.168.11.6 | sw02 | Cisco ISR 1100 (C1111-8P) を L2 スイッチ流用 | スイッチ不足対応、EVC + ESM で VLAN トランク収容 |
| 192.168.11.7–.9 | — | 予約 (将来の静的割り当て用) | |

## DHCP プール (.100 – .199)

AP (WLC 非管理) およびその他の機器向け。

| 項目 | 値 |
|------|-----|
| レンジ | 192.168.11.100 – 192.168.11.199 |
| リース時間 | 1h (デフォルト) / 2h (最大) |
| DNS サーバー | 192.168.11.1 |
| デフォルトゲートウェイ | 192.168.11.1 |

## 未使用 / 予約

| レンジ | 用途 |
|--------|------|
| .7–.9 | 将来の静的割り当て予約 |
| .10 – .99 | 未使用 |
| .200 – .254 | 未使用 |
